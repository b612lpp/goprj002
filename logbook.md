...
хух. наигрался с тестами на столько что создание экземпляра домена пришлось перенести в юз кейс, валидатор переписать, meterreading расширить количеством передаваемых значений. голову делает бд в памяти, пустые стартовые значения немного мешают.

Но зато теперь один валидатор данных домена который подойдёт для любого счетчика и тесты к нему, хоть и не все но в текущем проекте достаточно

Пора прикрутить историю. Попробую через эвенты в отдельной хранилке, оберну в интерфейс, а потом чтонибудь прикручу отдельностоящее от бд. Логичный вопрос зачем тут ивент, но вроде как почти бесплатно ну и это сейчас в юз- кейсе только валидация и сохранение, а в планах вроде как уведомления.

пауза на проекте. надо поразбираться с матчастью.

алгоритмические задачкина марше. 
├───project 
│       ├───file.txt (19b)
│       └───gopher.png (70372b)
├───static 
...
вот такую красоту нарисовал. продолжу в отдельном проекте.